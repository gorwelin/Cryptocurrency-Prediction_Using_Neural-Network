...
def get_Xy_and_model_for_asset(asset_id):
    ...
    predicted_score = pd.read_csv('pred' + str(asset_id) + '.csv')
    pred_sc_np = predicted_score.to_numpy()
    
    #for i in range(len(predicted_score)):
    #    pred_sc_np[i] = 
    
    ax_left.plot(predicted_score.rolling(3 * 30 * 24 * 60) color = 'crimson', label = "Target WCorr")
    ...
    np.savetxt("df2" + str(asset_id) + ".csv", df2, delimiter=",")
