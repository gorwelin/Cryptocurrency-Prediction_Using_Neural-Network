...
test = dt.fread(filepath + 'cryptocurrency-extra-data-binance-coin/orig_example_test.jay').to_pandas()
sample_prediction_df = dt.fread(filepath + 'cryptocurrency-extra-data-binance-coin/orig_example_sample_submission.jay').to_pandas()
assets = dt.fread(filepath + 'cryptocurrency-extra-data-binance-coin/orig_asset_details.jay').to_pandas()
assets_order = dt.fread(filepath + 'cryptocurrency-extra-data-binance-coin/orig_supplemental_train.jay').to_pandas().Asset_ID[:N_ASSETS]
...
np.savetxt("true.csv", y_test, delimiter=",")
...
for i in range(N_ASSETS):
    ...
    np.savetxt("pred" + str(i) + ".csv", predictions[:, i], delimiter=",")
    ...
    print(f"{asset_name} true:{y_true}")
    
    print(f"{asset_name} predicted:{y_pred}")
    
    np.savetxt("true" + str(i) + ".csv", y_test[:, i], delimiter=",")
...
sup = dt.fread(filepath + 'cryptocurrency-extra-data-binance-coin/orig_supplemental_train.jay').to_pandas()[:WINDOW_SIZE * (N_ASSETS)]
...
example = dt.fread(filepath + 'cryptocurrency-extra-data-binance-coin/orig_example_test.jay').to_pandas()[:WINDOW_SIZE - 1]
